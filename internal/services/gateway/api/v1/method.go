...
type method struct {
	...
	// Объект, содержащий данные о лимитах
...
func (h *Handler) limits(item model.Limits) limits {
	return limits{
		Currency:  item.Currency,
		MinAmount: convert.CentsToBase(item.MinAmount),
		MaxAmount: convert.CentsToBase(item.MaxAmount),
	}
}
